...
@section('title', 'Signa Master')

@section('content')
    <div class="container mx-auto px-4 py-8 max-w-4xl">

        {{-- Form Tambah Pasien --}}
        <h2 class="text-3xl font-semibold mb-6 text-gray-800 border-b pb-2">Tambah Signa</h2>
        ...
        @endif
        <form action="{{ route('signa.store') }}" method="POST" class="space-y-6 bg-white p-6 rounded-lg shadow-md">
            @csrf

            <div>
                <label for="kode_signa" class="block text-sm font-medium text-gray-700 mb-1">Kode Signa</label>
                <input type="text" name="kode_signa" id="kode_signa" required
            ...
            <div>
                ...
                <input type="text" name="nama_signa" id="nama_signa" required
                    class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm
                       focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
        ...
        <div class="mt-12">
            ...
            <h3 class="text-2xl font-semibold mb-4 text-gray-800 border-b pb-2">Daftar Signa</h3>
            {{-- <form method="GET" action="{{ route('signa.index') }}" class="mb-6 mt-6 flex items-center gap-3">
            ...
            <table class="min-w-full border border-gray-300 rounded-md overflow-hidden bg-white shadow-sm">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="px-5 py-3 border-b text-left text-sm font-medium text-gray-700">Kode Signa</th>
                        <th class="px-5 py-3 border-b text-left text-sm font-medium text-gray-700">Nama Signa</th>
                        <th class="px-5 py-3 border-b text-left text-sm font-medium text-gray-700">Aksi</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($signa as $item)
                        <tr>
                            <td class="px-6 py-4">{{ $item->id }}</td>
                            <td class="px-6 py-4">{{ $item->kode_signa }}</td>
                            <td class="px-6 py-4">{{ $item->nama_signa }}</td>
                            <td class="px-6 py-4 flex items-center gap-4">
                                {{-- Tombol Edit --}}
                                <a href="{{ route('signa.edit', $item->id) }}" class="text-blue-600 hover:text-blue-800"
                                    title="Edit Data">
                                ...
                                {{-- Tombol Delete --}}
                                <form action="{{ route('signa.destroy', $item->id) }}" method="POST"
